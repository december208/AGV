...
{ 
    ...
    ros::Rate rate(25);
    ROS_INFO("Go Go %s!",goal->trigger.data);
    double angle=0;
    ...
    try{
        targetTF = tfBuffer.lookupTransform("robot",goal->trigger.data,ros::Time(0));
    }
    ...
    if(distError>distTolerance){
        if((50>distError>20)&&(goal->trigger.data != "target")){
            cmd_vel.data = "qc";
            cmd_pub.publish(cmd_vel);
